...
Node *Tree::newNode(string value){
...
void Tree::insert(Node *&root,string value){
...
void Tree::delNode(Node *&root,string target){
    //Base case:
    if(root==NULL) return;
...
//Searches our tree for a value. Returns true if the value is found.
//Otherwise, it returns false.
bool Tree::search(Node *root,string target){
    Node *temp=root;
    //Finding the value in our tree if it exists.
    while(temp!=NULL){
        //If the target is smaller, we go to the left.
        if(target<temp->data) temp=temp->left;
        //If the target is larger, we go to the right.
        else if(target>temp->data) temp=temp->right;
        //If we've found our target, we're done.
        else if(target==temp->data) return true;
    }
    //If we didn't find our target...
    return false;
}
...
int Tree::size(Node *root){
    ...
    //Base case:
    if(root==NULL) return 0;
    //If we have a valid node, we increment our size.
    numNodes++;
    //Adding the size of the left subtree to our total.
    numNodes+=size(root->left);
    //Adding the size of the right subtree to our total.
    numNodes+=size(root->right);
    //Returning our size.
    return numNodes;
}
